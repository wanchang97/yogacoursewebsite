# 那由他瑜伽 Nayota Yoga 

🌐 **Language / 语言 / Sprache:** **🇨🇳 中文** | [🇬🇧 English](/en/README.md) | [🇩🇪 Deutsch](/de/README.md)

欢迎来到 **那由他瑜伽 Nayota Yoga** 数字化资源空间。

如果您希望了解创建这个资源空间的初衷以及“那由他”和“无常”背后的故事，请翻阅 [关于我 (About Me)](zh/preface.md)。

本网站致力于整理与分享 10 周 Vinyasa 流瑜伽课程的理论内容、实践序列与精华总结。期待能给所有来到这里的您种下一颗独立修习瑜伽的种子。我们的最终目标是：无需外在的声音引导，在自身身心的指引和反馈里，规律、安静地去练习。循序渐进地感受身体空间的变化，直到能观察并拓展自己内心的空间。


<img src="zh/figures/nayota-logo.png" alt="Nayota Yoga Logo" style="max-width:min(100%, 720px); height:auto;" />

## 🎬 完整 Vinyasa Flow 串联视频（夏季学期结课版）

本学期课程已圆满结束！以下是**完整一小时含音乐的流瑜伽串联练习视频**，涵盖从调息热身到大休息的完整流程。欢迎课后反复练习，亦可作为之后学期的参考。

<div class="video-container">
  <iframe
    src="https://www.youtube.com/embed/KM9qZc9Znk4"
    title="Nayota Yoga – 完整 Vinyasa Flow 含音乐串联 Full Vinyasa Flow with Music"
    frameborder="0"
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
    allowfullscreen>
  </iframe>
</div>

> 💡 **练习建议**：跟随音乐，找到你自己的节奏，不必追求完美动作——感受呼吸与身体的连接才是核心。

**视频串联结构（共9段）：**

| 时序 | 内容 | 配乐 |
|------|------|------|
| 1 | 🧘 坐立调息（无音乐） | — |
| 2 | 🌱 坐立热身：侧展、颈侧、坐姿扭转 | — |
| 3 | 🐅 四角板凳热身：猫牛、穿针、虎式、首个下犬 | 音乐1 |
| 4 | 🌿 站立序列（左侧）：战士一/二、三角、新月、蜥蜴、鸽式 | 音乐2 |
| 5 | 🌿 站立序列（右侧，镜像） | 音乐3 |
| 6 | ⚖️ 平衡挑战：战士三、飞机式、鹰式 | 音乐4 |
| 7 | 🌊 坐立拉伸（左侧）：头碰膝、马里奇扭转、指南针、狂野式 | Respite |
| 8 | 🌊 坐立拉伸（右侧，镜像） | Respite |
| 9 | 🌙 大休息式（挺尸式） | Softhearted |

---

## 📚 夏季学期的课程结构
**那由他瑜伽 Nayota Yoga 10周大学体育课程**

前面部分是基础体式的练习。我们每节课讲会着重体会一个基础体式，并通过线下的串联感受在练习这些体式的过程中，我们的意识应该关注在哪些地方。通过刻意的练习变成一种习惯，最好变成顺其自然的反应。

1. 该体式究竟**是什么样子**：一张标准图示。
   
2. 该体式**背后思想**：因为标准是一个外在的平均值，而我们的身体在严格意义上都是非标准件，我们应该如何将体式为己所用，转化成适合自己的体式。那什么是适合自己的体式？一个判断的方法，是能否在这个体式保持顺畅均匀的呼吸。如果可以，那么可以在自己稳定的状态下进一步尝试更多的幅度。如果呼吸开始闭塞，开始憋气去完成一个动作，那此时这个动作就是对你的身体而言还没有完全准备好的动作。瑜伽的思想不是努力，不是结果，而是感受，是过程。我们头脑里首先可以有一个最终的目标形状，但是从自己目前的身体状态到这个目标体式的过程是瑜伽真正在意的。
   
3. **实践序列**：可以感受这种背后思想获得更多空间的实践序列。

---

> 请通过左侧导航栏开始您的学习旅程。强烈建议您在进入具体的体式前，先从 **[☯️ 瑜伽是什么及五大要素](zh/philosophy.md)** 开始阅读，构建练习的底层心法心智。

<br>

## 📖 参考文献
[donna]: https://www.masterclass.com/classes/donna-farhi-teaches-yoga-foundations "Donna Farhi Teaches Yoga Foundations"
[lili]: https://www.youtube.com/@yogalili6775 "Yoga Lili YouTube Channel"
[pocketyoga]: https://www.pocketyoga.com/ "Pocket Yoga"

